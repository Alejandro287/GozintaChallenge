...
public class Graph<V> {
     ...
     * adjacent vertices.
     */
    public Map<V,Set<V>> neighbors = new HashMap<V,Set<V>>();
    ...
    public boolean containsEdge (V from, V to) {
        return neighbors.get(from).contains(to);
    }
    ...
    public ArrayList<Stack<V>>  listaChain = new ArrayList<Stack<V>>();
    int counter = 0 ;
    ...
    public void toString3 (V n, ArrayList<LinkedList<V>> paths){
        Set<V> lista = new HashSet<>(neighbors.get(n));
        LinkedList<V> currentPath = paths.get(index);
        LinkedList<V> currentPath2 = (LinkedList<V>) paths.get(index).clone();
        currentPath.addFirst(n);
        currentPath2.addFirst(n);
        if (!lista.isEmpty()){
            int counter = 0;
            for (V v: lista) {
                toString3(v, paths);
            ...
            paths.set(index, (LinkedList<V>) currentPath.clone());
            currentPath.clear();
        }
    }

    public ArrayList<LinkedList<V>> toString2(V n){
